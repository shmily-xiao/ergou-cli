 ...
 * env.ERGOU_API_KEY, env.ANTHROPIC_AUTH_TOKEN). Those settings exist for
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // ~/.claude settings (apiKeyHelper, settings.env.ERGOU_API_KEY) MUST NOT
  ...
  const hasExternalApiKey =
    apiKeySource === 'ERGOU_API_KEY' || apiKeySource === 'apiKeyHelper'
...
export function getAnthropicApiKeyWithSource(
  ...
  // --bare: hermetic auth. Only ERGOU_API_KEY env or apiKeyHelper from
  ...
  if (isBareMode()) {
    if (process.env.ERGOU_API_KEY) {
      return { key: process.env.ERGOU_API_KEY, source: 'ERGOU_API_KEY' }
    }
    if (getConfiguredApiKeyHelper()) {
      return {
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      ...
      throw new Error(
        'ERGOU_API_KEY or CLAUDE_CODE_OAUTH_TOKEN env var is required',
      )
    }
  ...
  // Check for ERGOU_API_KEY before checking the apiKeyHelper or /login-managed key
  if (
    effectiveApiKeyEnv &&
